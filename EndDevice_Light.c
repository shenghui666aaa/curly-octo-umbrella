...
void SampleApp_Init( uint8 task_id )
{
  ...
  HalUARTWrite( 0, "Light Node Ready\r\n", osal_strlen("Light Node Ready\r\n") );

  HalAdcInit();

  // Configure P0_7 as analog input for photoresistor (AIN7)
  P0SEL |= BV(7);
  P0DIR &= ~BV(7);
  ADCCFG |= BV(7);
  ...
  HalLcdWriteString( "Light Node", HAL_LCD_LINE_1, WHITE, BLACK );
}
    ...
    while ( MSGpkt )
    {
      switch ( MSGpkt->hdr.event )
      {
        ...
        case ZDO_STATE_CHANGE:
          ...
          {
            HalLcdWriteString( "Light Online", HAL_LCD_LINE_1, WHITE, BLACK );
...
void SampleApp_SendSensorData( void )
{
    ...
    // 读 ADC（你板子 P0.7）
    adc_val = HalAdcRead(LIGHT_ADC_CH, HAL_ADC_RESOLUTION_10) >> 6;
    ...
    light = 100 - (uint8)(adc_val * 100 / 255);
...
void SampleApp_SendPeriodicMessage( void )
{
  ...
  if ( AF_DataRequest( &SampleApp_Periodic_DstAddr, &SampleApp_epDesc,
                       SAMPLEAPP_OKACK_CLUSTERID, 2, str, &SampleApp_TransID,
